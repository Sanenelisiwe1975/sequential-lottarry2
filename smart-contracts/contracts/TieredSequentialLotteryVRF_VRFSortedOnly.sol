...
/**
 * @title TieredSequentialLotteryVRF_Secure_VRFSorted
 * @dev Enhanced secure lottery contract with SORTED VRF numbers only
 * 
 * IMPORTANT: 
 * - Players MUST pick numbers in sequential order (position matters)
 * - Chainlink VRF numbers are SORTED in ascending order after generation
 * - This makes the winning numbers always in ascending order
 * 
 * Example:
 * - Player picks: [5, 12, 23, 34, 40, 45, 49] (MUST be in order)
 * - VRF generates: [34, 5, 49, 12, 23, 40, 8] (random)
 * - Contract sorts VRF: [5, 8, 12, 23, 34, 40, 49] (ascending)
 * - Matching: [5,12,23,34,40,45,49] vs [5,8,12,23,34,40,49]
 *   Position 0: 5==5 ✓
 ...
 * - SORTED VRF ONLY: Winning numbers always ascending
 ...
 */
contract TieredSequentialLotteryVRF_Secure_VRFSorted is 
    VRFConsumerBaseV2, 
    ReentrancyGuard, 
    Pausable,
    Ownable 
{
     ...
     * Player numbers are stored AS-IS (not sorted)
     * Player must pick numbers in the order they want to match
     */
        ...
        validAddress(msg.sender)
    {
        ...
        Ticket memory newTicket = Ticket({
            player: msg.sender,
            numbers: numbers,  // Stored exactly as player entered
    ...
    /**
     * @dev Sort an array of 7 numbers in ascending order
     * Uses insertion sort - efficient for small arrays
     * Gas cost: ~3,000-5,000 gas
     */
     ...
     * IMPORTANT: VRF numbers are SORTED in ascending order
     * Player numbers are compared AS-IS (not sorted)
     */
    function fulfillRandomWords(
        ...
        // Generate numbers from Chainlink VRF
        uint8[7] memory tempNumbers;
        for (uint256 i = 0; i < NUMBERS_COUNT; i++) {
            tempNumbers[i] = uint8((randomWords[i] % MAX_NUMBERS) + 1);
        }
        
        // SORT THE VRF NUMBERS IN ASCENDING ORDER
        // This is the ONLY sorting - player numbers are NOT sorted
        ...
        // Process winners
        // Player numbers (not sorted) vs Winning numbers (sorted)
     ...
     * Player numbers: AS-IS (not sorted)
     * Winning numbers: SORTED (ascending order)
     */
